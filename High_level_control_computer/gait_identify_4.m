...
med_filt_width = 20;
clean_data_1 = medfilt1(flex_1, med_filt_width,'truncate');
clean_data_2 = medfilt1(flex_2, med_filt_width,'truncate');
clean_data_3 = medfilt1(flex_3, med_filt_width,'truncate');
clean_data_4 = medfilt1(flex_4, med_filt_width,'truncate');
...
range_flex_1 = max(clean_data_1) - min(clean_data_1);
range_flex_2 = max(clean_data_2) - min(clean_data_2);
range_flex_3 = max(clean_data_3) - min(clean_data_3);
range_flex_4 = max(clean_data_4) - min(clean_data_4);

% Adding noise to find peaks
r = (0.0001 + (0.003 - 0.001) * rand(1, size(all_data,1)))';
% 
...
clean_data_1_new = clean_data_1 + r;
clean_data_2_new = clean_data_2 + r;
clean_data_3_new = clean_data_3 + r;
clean_data_4_new = clean_data_4 + r;
...
threshold_1 = max(clean_data_1_new) - 0.92*range_flex_1;
threshold_2 = max(clean_data_2_new) - 0.92*range_flex_2;
threshold_3 = max(clean_data_3_new) - 0.92*range_flex_3;
threshold_4 = max(clean_data_4_new) - 0.83*range_flex_4;
...
for nn = 1:size(gaits_switch,1)
    for mm = 1:size(gaits_switch,2)
        ...
        if size(gaits_switch{nn,mm},1) < 11
            gaits_switch{nn,mm} = [];
        end
    end
end
